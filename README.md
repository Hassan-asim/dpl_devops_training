...
This repository now tracks my DevOps practice in **date-wise folders** so you can review each dayâ€™s progress, artifacts, and supporting documents in one place. Iâ€™ll continue to mirror this structure for future evaluations and test projects you share in the same folders.

## ðŸ“‚ Repository Structure

- `YYYY-MM-DD/` â€“ A dedicated folder for that dayâ€™s work. Each contains:
  - `README.md` â€“ the full daily report (study log, challenges, fixes, screenshots).
  - `images/` â€“ screenshots embedded in the daily README.
...
| 2026-01-20 | **Task 5: Full-Stack Cloud Architecture:** Implemented production-ready three-tier architecture with React frontend (S3/CloudFront), NestJS backend (ECS Fargate), PostgreSQL database (RDS Multi-AZ), comprehensive CI/CD pipeline (GitLabâ†’CodeBuildâ†’ECRâ†’ECS), security (IAM/Secrets Manager), and monitoring (CloudWatch/X-Ray); complete implementation checklist and database connectivity verification saved to `2026-01-20/images/`. |
...
| 2026-01-13 | **AWS Skill Builder Labs Completion:** Completed four hands-on labs â€” Networking Concepts, File Systems in the Cloud, First NoSQL Database, and Cloud Economics; verified certificates and digital badges; lab completion screenshots and certificates saved to `2026-01-13/images/`. |
...
| 2026-01-07 | **Certifications completed:** Completed four courses â€” Advanced Linux Commands; Configuration Management & Cloud; Development with Amazon Q; Google AI Essentials. All certificates and screenshots saved to `2026-01-07/images/`. |
| 2026-01-06 | **CDK course completed + EC2 website hosted:** Completed the CDK course and lab (`hello-cdk/`), launched an EC2-hosted static website with Nginx and Certbot (Letâ€™s Encrypt); Elastic IP allocated and associated â€” awaiting DNS A record propagation to finalize the certificate. Screenshots and evidence: `2026-01-06/images/`. |
| 2026-01-05 | **Static site deployed + DNS fix:** Added website code to `static-website/` and `static-website-with-domain.yaml`; uploaded screenshots to `2026-01-05/images/`; fixed site access issue by changing VPN DNS to a public resolver (1.1.1.1) and flushing cache â€” site now works on and off VPN. |
| 2026-01-02 | **Static site w/ domain:** Requested ACM certificate and deployed domain-backed static site using `static-website-with-domain.yaml`; DNS validation pending â€” artifacts in `2026-01-02/`. |
...
| 2025-12-30 | **Learned:** AWS Single Sign-On (IAM Identity Center) â€” read official docs and watched an in-depth YouTube video; notes in `2025-12-30/` (no images). |
| 2025-12-29 | **Certificates added:** EBS, EBS Performance Optimization, and Security Best Practices (PNG) saved in `2025-12-29/` â€” daily report created with evidence in `images/`. |
...
| 2025-12-22 | Watched **~half** of the Amazon EC2 course (through Topic 6 â€” Launch Template); full course outline and video link added (`https://www.youtube.com/watch?v=4dscVzCaXCU`); no screenshots â€” planning hands-on labs next; details in `2025-12-22/` |
| 2025-12-19 | AWS architecture course Modules 6â€“10 completed with detailed part-wise screenshots; certificate of completion saved; evidence in `2025-12-19/` |
| 2025-12-18 | AWS architecture course Modules 2â€“5 completed with detailed part-wise screenshots; STS course fully completed with certificate; evidence in `2025-12-18/` |
| 2025-12-17 | New AWS architecture course started (parts 1â€“16) plus STS segment; Rebel Capture â€“ DPL Webcam Prototype repo reviewed and app UI screens captured; evidence in `2025-12-17/` |
...
| 2025-12-12 | AWS Cloud Practitioner Essentials completed with certificate; Mastering AWS CDK completed with certificate; modules 10â€“13 evidence in `2025-12-12/` |
...
| 2025-12-10 | AWS Cloud Practitioner Essentials Module 9 completed; Module 10 started; Mastering AWS CDK â€“ Coding Cloud Architectures Modules 6â€“8 completed; rewatched Kubernetes microservices video; evidence in `2025-12-10/` |
| 2025-12-09 | AWS Cloud Practitioner Essentials Modules 4â€“8 completed; Mastering AWS CDK Modules 4â€“5 completed; Kubernetes microservices fundamentals video (deployments, scaling, ingress); evidence in `2025-12-09/` |
| 2025-12-08 | AWS Cloud Practitioner Essentials Modules 1â€“4 completed; Mastering AWS CDK Modules 1â€“3 completed; AWS CLI profile configured and CDK installed |
...
navigate into the date folder you want to reviewâ€”for example:

```
cd 2025-11-21
```

Youâ€™ll find the detailed README plus every artifact referenced.

## ðŸ”— Latest Live Projects

- `2025-12-4` â€” Meeting Room Booker (live): https://room-booker-dpl.vercel.app/  
...
- Iâ€™ll keep uploading new days exactly the same way.
- Feel free to drop evaluations or project briefs inside the relevant date folder; Iâ€™ll respond in-place.

Let me know if youâ€™d prefer a different naming scheme or extra metadata (e.g., time logs, task checklists) in each folder.
